...
# Set window title
*.title = iDroid Error Reporter

# Introductory text
tb.type = text
tb.default = Welcome to the AndersenTech iDroid Reporter! Please Choose the Reason of Reporting Below!

# Define radiobuttons
rb.type = radiobutton
rb.label = Errors
rb.option = Installer - General
rb.option = Installer - OpeniBoot (Fresh)
rb.option = Installer - OpeniBoot (Upgrade)
rb.option = Android System
rb.option = Uninstall
rb.option = Other
rb.default = Installer - General
...
if [ "$rb" = "Installer - General" ]; then
	SH err_1600.sh;
fi
if [ "$rb" = "Installer - OpeniBoot (Fresh)" ]; then
	SH err_1601.sh;
fi
if [ "$rb" = "Installer - OpeniBoot (Upgrade)" ]; then
	SH err_1602.sh;
fi
if [ "$rb" = "Android System" ]; then
	SH err_1603.sh;
fi

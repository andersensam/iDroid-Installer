...
echo "Welcome to the AndersenTech iDroid Installer"

echo "Checking for Updates!"

sh ./UpdateChecker.sh

echo "Killing iTunes + Helper"
...
# Set window title
*.title = iDroid Installer

# Introductory text
tb.type = text
tb.default = Welcome to the AndersenTech iDroid Installer! For the OpeniBoot Install, Choose Fresh if You Have No Other Versions of OpeniBoot Installed. If You Have Another Version Please Select Upgrade! You MUST Install the Dependencies Before Running the Installer!

# Define radiobuttons
rb.type = radiobutton
rb.label = Choose Your Mode
rb.option = Installer
rb.option = Install OpeniBoot (Fresh)
rb.option = Install OpeniBoot (Upgrade)
rb.option = Exit Recovery
rb.option = Install Dependencies
rb.option = Uninstall
rb.option = Error Reporting
rb.default = Installer
...
if [ "$rb" = "Installer" ]; then 
	rm $HOME/.ssh/known_hosts
	SH ./Uplink.sh;
fi

if [ "$rb" = "Install OpeniBoot (Fresh)" ]; then
	SH Uplink_OIB_Fresh.sh;
fi
if [ "$rb" = "Install OpeniBoot (Upgrade)" ]; then
	SH Uplink_OIB_Upgrade.sh;
fi
if [ "$rb" = "Exit Recovery" ]; then
	open app/RecoveryOUT.app;
fi
if [ "$rb" = "Install Dependencies" ]; then
	SH root_pw.sh;
fi
if [ "$rb" = "Uninstall" ]; then
	SH Uplink_REMV.sh;
fi
if [ "$rb" = "Error Reporting" ]; then
	SH err_report.sh;
fi
